...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function showView(viewName, title) {
    ...
    if (viewName === 'kanban') {
      activeView = document.getElementById('kanban-view');
    } else if (viewName === 'code') {
      activeView = document.getElementById('kan8-view');
    } else if (viewName === 'webpagecourse') {
      activeView = document.getElementById('webpagecourse-view');
    } else {
      // For all other menu items, create a temporary blank view with placeholder
      activeView = document.createElement('div');
      activeView.className = 'view-container active';
      const leftCol = document.createElement('div');
      leftCol.className = 'left-column';
      activeView.appendChild(leftCol);

      const cleanTitle = title || viewName.charAt(0).toUpperCase() + viewName.slice(1);
      leftCol.appendChild(createPlaceholder(cleanTitle));

      document.querySelector('.page-container').appendChild(activeView);
    }
  ...
  navLinks.forEach(link => {
    link.addEventListener('click', (e) => {
      ...
      if (text.includes('Kanban board')) view = 'kanban';
      else if (text.includes('Code')) view = 'code';
      else if (text.includes('WebpageCourse')) view = 'webpagecourse';
      else title = text
      ...
      showView(view || title.toLowerCase(), title);
    });
  });
...
//     } else if (viewName === 'webpagecourse') {
//       document.getElementById('webpagecourse-view')?.classList.add('active');
//     }
